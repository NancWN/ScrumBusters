...
def load_data(data_root:str='data/engineered/',dataset_id:str='newest'):
    try:
        if dataset_id == 'newest':
            newest_file = max(os.listdir(data_root))
            data = pd.read_csv(data_root+newest_file)
        else:
            data = pd.read_csv(f'{data_root}{dataset_id}.csv')
        return data, newest_file.replace('.csv','')
    except Exception as e:
        logging.error('loading data failed. Error: %s', e)